...
(function(window) {
  if ("undefined" !== typeof CDC && "undefined" !== typeof CDC.ABTest) {
    ...
    var runAbTest = function(item, opts) {
      ...
      if (
        item.attr("data-percent-variant") &&
        !isNaN(parseInt(item.attr("data-percent-variant")))
      ) {
      ...
      var queryParms = window.location.href
      ...
      if (highlight) {
        $(opts.selector).attr(
          "style",
          "border: 4px solid rgb(201, 34, 34)!important; padding: 4px!important"
        );
      }
      ...
      $(opts.selector)
        ...
        .on("click", function(e) {
          ...
          console.log(
            'tag.hasClass("ab-track-link")',
            tag.hasClass("ab-track-link")
          );
          var trackLink = false;
          if (
            (1 < anchorCount && 0 === abAnchorCount) ||
            (1 === anchorCount && 0 === abAnchorCount)
          ) {
            trackLink = true;
          } else if (tag.is("a")) {
          ...
          if (trackLink) {
            ...
            s.events = "event2";
            s.eVar8 =
              $(opts.selector).attr("data-abid") +
              "_" +
              $(opts.selector).attr("data-ab-option");
            s.linkTrackEvents = "event2";
            s.linkTrackVars =
              "prop2,prop31,prop46,prop18,prop64,pageName,events,eVar8";
    ...
    if (testActive && CDC.ABTest.TEST_TYPE_REDIRECT === testType) {
      ...
      if (currentUrl === variantUrl && "" === document.referrer) {
        ...
        if (redirect.sticky) {
          ...
          abCookies = cookies.filter(function(cookieItem) {
            return (
              -1 <
              cookieItem.indexOf(
                "CDC_ABTest_Redirect_" + testId + "_" + redirect.id
              )
            );
          });
          if (0 < abCookies.length) {
            ...
            if (0 < abCookie.length) {
              ...
              useVariant =
                abCookieValue === variantUrl
                  ? true
                  : abCookieValue === baselineUrl;
            }
          }
        }
        ...
        if (
          ...
          console.log(
            "On a redirect variant page with useVariant = " + useVariant
          );
      ...
      if (window.location.href.toLowerCase() !== baselineUrl.toLowerCase()) {
        ...
        if (-1 < baselineRelUrl.indexOf("//")) {
          baselineRelUrl = baselineRelUrl.substring(
            baselineRelUrl.indexOf("//") + 2
          );
          baselineRelUrl = baselineRelUrl.substring(
            baselineRelUrl.indexOf("/")
          );
        }

        var currentRelUrl = window.location.href;
        if (-1 < currentRelUrl.indexOf("//")) {
          currentRelUrl = currentRelUrl.substring(
            currentRelUrl.indexOf("//") + 2
          );
          currentRelUrl = currentRelUrl.substring(currentRelUrl.indexOf("/"));
        }

        if (0 < $('nav ul li a[href="' + baselineRelUrl + '"]').length) {
          $('nav ul li a[href="' + baselineRelUrl + '"]').attr(
            "href",
            currentRelUrl
          );
        }
      ...
      if (0 < cdcAbDivs.length) {
        ...
        $("div[data-abid]").each(function() {
          ...
          if ("" !== abId) {
            ...
            useVariant =
              Math.round(100 * Math.random()) > 100 - options.percentVariant;
            ...
            if (
              "" !== abCookieValue &&
              (options.sticky || "true" === options.sticky)
            ) {
